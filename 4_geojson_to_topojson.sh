...
do
  ...
  do
    ...
    do
      ...
      geo2topo -q 20000 nutsrg=$dir"/RG/"$level".json" nutsbn=$dir"/BN/"$level".json" cntrrg=$dir"/RG/CNTR.json" cntrbn=$dir"/BN/CNTR.json" > $dir"/"$level".topojson"
      #q small means strong simplification
    done
  done
done
